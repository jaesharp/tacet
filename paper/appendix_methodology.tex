...
\end{equation}
This is solved by deterministic 1D root-finding using Monte Carlo integration (50,000 samples). We use $\pi_0 = 0.62$ rather than 0.5 to avoid implying the prior places equal weight on extreme quantile differences across all nine dimensions—which would be overly pessimistic, given that the maximum of nine independent samples naturally has higher exceedance probability than a single sample. This value expresses genuine uncertainty without strong prior bias toward either outcome. The resulting prior expresses genuine uncertainty: it does not strongly favor either ``leak'' or ``no leak'' before seeing data.
...
We approximate the posterior $p(\delta \mid \Delta)$ using a Gibbs sampler over $(\delta, \lambda, \kappa)$. Each iteration samples:
\begin{enumerate}
...
We compute the integrated autocorrelation time (IACT) $\hat{\tau}$ using the Geyer initial monotone sequence (IMS) algorithm~\cite{geyer1992}. This estimates how many nominal samples equal one independent sample:
\begin{equation}
